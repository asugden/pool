...
class Clustering(object):
    def __init__(self, data):
        ...
        self.vdriven = {}
        for cs in ['plus', 'neutral', 'minus', 'hofer']:
            self.vdriven[cs] = self.analysis('visually-driven-%s' % cs) > 50
        self.vdriven['spontaneous'] = self.vdriven['hofer']

        self.out['cluster-n'] = self.clustern('hofer')

        nclusters = 0
        nclusters1 = 0
        for cs in ['plus', 'neutral', 'minus', 'hofer']:
            nodes, edges, ncells = self.nodeedges(cs)
            self.out['graph-clustering-%s' % cs] = self.nodeclusters(nodes, edges, ncells)
            self.out['graph-clustering-within-%s' % cs] = self.localnodeclusters(nodes, edges, ncells, within=True)
            self.out['graph-clustering-between-%s' % cs] = self.localnodeclusters(nodes, edges, ncells, within=False)

            self.out['n-graph-clusters-%s' % cs] = self.getgroups(cs)
            self.out['n-graph-clusters-1-%s' % cs] = self.getgroups(cs, 0)
            nclusters += self.out['n-graph-clusters-%s' % cs]
            nclusters1 += self.out['n-graph-clusters-1-%s' % cs]

            vdriven = float(np.sum(self.vdriven[cs]))
            if vdriven > 0:
                self.out['frac-vdrive-clusters-%s' % cs] = divzero(vdriven, self.out['n-graph-clusters-%s' % cs])
                self.out['frac-vdrive-clusters-1-%s' % cs] = divzero(vdriven, self.out['n-graph-clusters-1-%s' % cs])
            else:
                self.out['frac-vdrive-clusters-%s'%cs] = np.nan
                self.out['frac-vdrive-clusters-1-%s'%cs] = np.nan

        for cs in ['plus', 'neutral', 'minus', 'hofer']:
            if nclusters == 0:
                self.out['frac-graph-clusters-%s'%cs] = 0
            else:
                self.out['frac-graph-clusters-%s' % cs] = float(self.out['n-graph-clusters-%s' % cs])/nclusters

            if nclusters1 == 0:
                self.out['frac-graph-clusters-1-%s' % cs] = 0
            else:
                self.out['frac-graph-clusters-1-%s' % cs] = float(self.out['n-graph-clusters-1-%s' % cs])/nclusters1
    ...
    requires = ['']
    sets = [['graph-clustering-%s' % cs,
             'graph-clustering-within-%s'%cs,
             'graph-clustering-between-%s'%cs,
             'n-graph-clusters-%s' % cs,
             'frac-graph-clusters-%s' % cs,
             'frac-vdrive-clusters-%s' % cs,
             'frac-vdrive-clusters-1-%s' % cs,
             'n-graph-clusters-1-%s' % cs,
             'frac-graph-clusters-1-%s' % cs] for cs in ['plus', 'neutral', 'minus', 'hofer']] + \
            ['cluster-n']
    across = 'day'
    updated = '180328'

    depends_on = ['noise-correlation-%s' % cs for cs in ['plus', 'neutral', 'minus']] + \
    ...
    def nodeedges(self, cs=''):
        ...
        if len(cs) > 0 and cs != 'spontaneous':
            corr = self.analysis('noise-correlation-%s' % cs)
