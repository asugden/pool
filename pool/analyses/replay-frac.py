...
class PairReplays(object):
    def __init__(self, data):
        ...
        for cs in ['plus', 'neutral', 'minus']:
            count = self.analysis('replay-count-0.1-%s' % cs)

            reps = 0
            for run in data['sated']:
                reps += len(outfns.emptynone(self.analysis('event-peaks-run%i-%.2f-%s' % (run, 0.1, cs))))

            if count is None:
                self.out['replay-frac-0.1-%s'%cs] = np.nan
            else:
                self.out['replay-frac-0.1-%s' % cs] = count.astype(np.float64)/reps
    ...
    requires = ['classifier']
    sets = ['replay-frac-0.1-%s' % cs for cs in ['plus', 'neutral', 'minus']]
    across = 'day'
    updated = '180302'
